...
Build status:
[![Workflow executed at each push](https://github.com/fchastanet/bash-tools/actions/workflows/push_branch.yml/badge.svg?branch=master)](https://github.com/fchastanet/bash-tools/actions/workflows/push_branch.yml)
...
```text
Description: Execute a query on multiple databases in order to generate a report with tsv format,
query can be parallelized on multiple databases
...
Usage: dbQueryAllDatabases <query|queryFile> [-d|--dsn <dsn>] [-q|--query] [--jobs|-j <jobsCount>] [--bar|-b]
    ...
    -b|--bar              Show progress as a progress bar. In the bar is shown: % of jobs
...
List of available dsn:
       ...
       - default.remote
       - localhost-root
List of available queries (default dir /bash/conf/dbQueries can be overridden in home dir /home/www-data/.bash-tools/dbQueries):
       - databaseSize
```
...
Usage: dbScriptAllDatabases [-j|--jobs <numberOfJobs>] [-o|--output <outputDirectory>]
[-d|--dsn <dsn>] [-v|--verbose] [-l|--log-format <logFormat>] [--database <dbName>]
<scriptToExecute>
[optional parameters to pass to the script]
    ...
    --database <dbName>           if provided will check only this db, otherwise script will
    be executed on all dbs of mysql server
    ...
    -l|--log-format <logFormat>   if log provided, will log each db result to log file, can
    be one of these values (none, log) (default: none)
...
Example: script conf/dbScripts/extractData.sh
    executes query databaseSize (see conf/dbQueries/databaseSize.sql) on each db and log the
    result in log file in default output dir, call it using
...
Usage: dbImport -a|--from-aws <fromDbS3Filename> [<targetDbName>]
    ...
    -p|--profile profileName    the name of the profile to use in order to include or exclude tables
        (if not specified /home/www-data/.bash-tools/dbImportProfiles/default.sh
        is used if exists otherwise /bash/conf/dbImportProfiles/default.sh)
...
List of available profiles (default profiles dir /bash/conf/dbImportProfiles can be overridden in home profiles /home/www-data/.bash-tools/dbImportProfiles):
       - all
       - default
       - none
List of available dsn:
...
List of available profiles (from /bash/conf/cliProfiles and can be overridden in /home/www-data/.bash-tools/cliProfiles):
       - default
       - mysql
       - mysql.remote
       - node
...
mysqldump --skip-add-drop-table --skip-add-locks --skip-disable-keys --skip-set-charset \
   --user=root --password=root --no-data skills | mysql2puml
...
$(mysql --host=127.0.0.1 --port=3345 --user=root --password=root skills \
  -Bse "show tables like 'core\_%'") | grep -v '^\/\*![0-9]\{5\}.*\/;$' > doc/schema.sql
```
