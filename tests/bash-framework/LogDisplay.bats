...
@test "Log::displayDebug activated with env var" {
    BASH_FRAMEWORK_INITIALIZED=0 BASH_FRAMEWORK_DISPLAY_LEVEL=${__LEVEL_DEBUG} source "$(cd "$( readlink -f "${BATS_TEST_DIRNAME}/../..")" && pwd)/bash-framework/_bootstrap.sh" || exit 1
    run assertDisplayLogs ${__LEVEL_DEBUG}
    [[ "${status}" == "0" ]]
}
