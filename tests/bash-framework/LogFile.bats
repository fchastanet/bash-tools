...
@test "${BATS_TEST_FILENAME#/bash/tests/} Log::logDebug activated with env var" {
    BASH_FRAMEWORK_LOG_FILE="${logFile}"
    BASH_FRAMEWORK_INITIALIZED=0 BASH_FRAMEWORK_LOG_LEVEL=${__LEVEL_DEBUG} source "$(cd "${BATS_TEST_DIRNAME}/../.." && pwd)/bash-framework/_bootstrap.sh" || exit 1
    assertFileLogs ${__LEVEL_DEBUG}
    [[ "$?" == "0" ]]
}
...
@test "${BATS_TEST_FILENAME#/bash/tests/} Log::logInfo activated with env var" {
    BASH_FRAMEWORK_LOG_FILE="${logFile}"
...
@test "${BATS_TEST_FILENAME#/bash/tests/} Log::logSuccess activated with env var" {
    BASH_FRAMEWORK_LOG_FILE="${logFile}"
...
@test "${BATS_TEST_FILENAME#/bash/tests/} Log::logWarning activated with env var" {
    BASH_FRAMEWORK_LOG_FILE="${logFile}"
...
@test "${BATS_TEST_FILENAME#/bash/tests/} Log::logError activated with env var" {
    BASH_FRAMEWORK_LOG_FILE="${logFile}"
...
@test "${BATS_TEST_FILENAME#/bash/tests/} log off with env var" {
    BASH_FRAMEWORK_LOG_FILE="${logFile}"
